...
class GetLeaderBoardTest extends WebTestCase
{
    ...
    public function testGetLeaderBoardWithApiToken(): void
    {
        ...
        self::assertEquals('jerome.dumas', $responseToAssert[2]->username);
        self::assertEquals(18.76, $responseToAssert[2]->score);

        self::assertEquals(
            (new DateTime())->format('d/m/Y'),
            (new DateTime($responseToAssert[2]->createdAt->date))->format('d/m/Y')
        );

        self::assertEquals(
            (new DateTime())->format('d/m/Y'),
            (new DateTime($responseToAssert[2]->updatedAt->date))->format('d/m/Y')
        );
    }
}
